  ...
  def actors_payment(car)
    ...
    ACTORS.each do |actor|
      ...
      when 'owner'
        ...
        type = 'credit'
      when 'insurance'
        amount = total * 0.3 * 0.5
        type = 'credit'
      when 'assistance'
        amount = duration_in_days * 100
        type = 'credit'
      when 'drivy'
        amount = total * 0.3 * 0.5 - duration_in_days * 100
        type = 'credit'
      end
      payments << { who: actor, type: type, amount: amount.to_i }
    end
    payments
  end
...
output = {
  rentals: rentals.map do |rental|
    ...
    { id: rental.id, actions: rental.actors_payment(car) }
  end
}
